...
func NewReplicaServer(volumeName, instanceName string, s *replica.Server) *grpc.Server {
	...
	// weft-block extension : streaming snapshot read for our own backup
	// pipeline (BackupTarget abstraction in pkg/backuptarget — independent
	// of Longhorn's sync-agent backup flow). See weftsnap_server.go.
	weftsnap.RegisterSnapshotReaderServer(server, newSnapshotReaderServer(volumeName, s))
	return server
}
