...
import (
	...
	"github.com/openweft/weft-block/pkg/backingfile"
	"github.com/openweft/weft-block/pkg/types"
	"github.com/openweft/weft-block/pkg/util"

	diskutil "github.com/openweft/weft-block/pkg/util/disk"
)
...
// SnapshotRange is one (offset, size) window that differs between two
// snapshots — emitted by CompareSnapshot. Mirrors the wire shape in
// pkg/replica/rpc/weftsnap.Range, but kept package-local so this
// pure-replica API doesn't pull the rpc package into its import graph.
type SnapshotRange struct {
	Offset int64
	Size   int64
}

// CompareSnapshot walks the replica's sector→file mapping (volume.location)
// and returns the byte windows where snapshotName differs from
// parentSnapshotName, aligned to blockSize.
//
// This is the public, lifecycle-free analogue of
// BackupStatus.CompareSnapshot — same logic, no cwd/OpenSnapshot
// dependency. The Replica must already be open on snapshotName (the
// caller opens it via NewReadOnly) ; parentSnapshotName must live in
// the chain (i.e. be one of the activeDiskData entries below
// snapshotName), else findDisk returns 0 and we surface a clear error.
//
// Empty parentSnapshotName means "compare against nothing" — every
// allocated sector below snapshotName's index ends up in the result,
// effectively encoding "the full sparse footprint of the snapshot
// relative to the empty volume". Callers wanting a true full backup
// should use Read instead — CompareSnapshot with an empty parent is
// the equivalent operation but stays sparse over the holes.
//
// blockSize must be > 0. Practical values : 1 MiB to 16 MiB ; smaller
// blocks emit more ranges + finer granularity, larger blocks ship
// more sector overlap but fewer chunks.
//
// Concurrency : takes r.RLock (read-only walk over the location
// table). Safe to call concurrently with other readers ; safe NOT to
// call concurrently with writers (the replica must be read-only here).
func (r *Replica) CompareSnapshot(snapshotName, parentSnapshotName string, blockSize int64) ([]SnapshotRange, error) {
	if snapshotName == "" {
		return nil, fmt.Errorf("snapshot name is required")
	}
	if blockSize <= 0 {
		return nil, fmt.Errorf("block size must be > 0, got %d", blockSize)
	}
	r.RLock()
	defer r.RUnlock()
	from := r.findDisk(snapshotName)
	if from <= 0 {
		return nil, fmt.Errorf("snapshot %q not found in chain", snapshotName)
	}
	// Empty parent ⇒ to=0 (the nil-file sentinel). Then the walk
	// catches every sector where val ∈ [1, from], i.e. anything ever
	// touched at or below snapshotName.
	to := 0
	if parentSnapshotName != "" {
		to = r.findDisk(parentSnapshotName)
		if to <= 0 {
			return nil, fmt.Errorf("parent snapshot %q not found in chain", parentSnapshotName)
		}
		if to > from {
			return nil, fmt.Errorf("parent snapshot %q sits above %q in chain (from=%d to=%d) — backwards diff isn't a thing", parentSnapshotName, snapshotName, from, to)
		}
	}
	// Make sure the location table is fully populated before the
	// walk. Preload(false) maps sectors that haven't been read yet —
	// without it, location[i]=0 could mean either "nil sector" or
	// "not yet known", which would silently miss changes.
	if err := r.Preload(false); err != nil {
		return nil, fmt.Errorf("preload location table: %w", err)
	}
	sectorSize := r.volume.sectorSize
	if sectorSize <= 0 {
		return nil, fmt.Errorf("replica has zero sector size — likely uninitialized")
	}
	// Emit one Range per blockSize-aligned window that contains at
	// least one sector with location ∈ (to, from]. We merge sectors
	// that fall into the same block by tracking the last-emitted
	// block offset.
	var out []SnapshotRange
	lastOffset := int64(-1)
	for i, val := range r.volume.location {
		if val <= byte(from) && val > byte(to) {
			offset := int64(i) * sectorSize
			offset -= offset % blockSize
			if offset != lastOffset {
				out = append(out, SnapshotRange{Offset: offset, Size: blockSize})
				lastOffset = offset
			}
		}
	}
