...
// Dir returns the replica's state directory. Exposed (was an internal field
// in upstream Longhorn) so weft-block's snapshot-read RPC can mount a fresh
// read-only Replica handle pointing at the same on-disk image without
