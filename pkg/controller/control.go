...
func (c *Controller) Snapshot(inputName string, labels map[string]string, shouldFreeze bool) (name string, err error) {
	...
	// The filesystem-freeze code path (k8s.io/mount-utils + a bind-mount dance
	// inside the longhorn-manager container) was dropped with the move to NBD:
	// the freeze relied on k8s container-namespace assumptions weft doesn't
	// have. We always fall back to a best-effort sync before snapshotting,
	// which is what upstream Longhorn does when no eligible mount is found.
	if shouldFreeze {
		log.Debug("shouldFreeze requested but filesystem freeze is not supported in this fork; using sync fallback")
	}
	log.Info("Requesting system sync before snapshot")
	if err := lhns.Sync(); err != nil {
		log.WithError(err).Errorf("WARNING: failed to sync before snapshot; continuing")
	}
...
func (c *Controller) StartFrontend(frontend string) error {
	...
	c.frontend = f
	// Release write lock BEFORE startFrontend → frontend.Startup. The NBD
	// frontend spawns in-process goroutines that immediately receive a kernel
	// udev probe ReadAt; controller.ReadAt acquires RLock which would
	// otherwise deadlock against this writer holding the lock through the
	// entire Startup chain (which never returns until udev probe is served).
	// Upstream's iSCSI frontend doesn't hit this because tgt runs OOP.
	c.Unlock()
...
func (c *Controller) Start(volumeSize, volumeCurrentSize int64, addresses ...string) error {
	...
	for _, address := range addresses {
		...
		// If the instance manager crashes during the execution of [this code block](https://github.com/openweft/weft-block/blob/v1.5.1/pkg/sync/sync.go#L435-L446)
...
func (c *Controller) metricsStop() {
	c.metricsLock.Lock()
	defer c.metricsLock.Unlock()
	if c.metricsStopCh != nil {
...
// (tryFreeze and attemptBindMountFilesystem removed with the mount-utils strip;
// see Snapshot — we now always sync-and-snapshot rather than freezing.)
